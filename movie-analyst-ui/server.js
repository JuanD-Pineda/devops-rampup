...
// The homepage route of our application does not interface with the MovieAnalyst API and is always accessible. We'll simply render the index.ejs view.
...
app.get('/movies', function(req, res){
  request
    .get(`http://${backend_url}/api/movies`)
...
app.get('/authors', function(req, res){
  request
    .get(`http://${backend_url}/api/reviewers`)
...
app.get('/publications', function(req, res){
  request
    .get(`http://${backend_url}/api/publications`)
    .end(function(err, data) {
      if(data.status == 403){
        res.send(403, '403 Forbidden');
      } else {
...
app.get('/pending', function(req, res){
  request
    .get(`http://${backend_url}/api/pending`)
    .end(function(err, data) {
      if(data.status == 403){
        res.send(403, '403 Forbidden');
      }
